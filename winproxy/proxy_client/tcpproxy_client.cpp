	...
	public:
		...
		void start(const std::string& upstream_host, unsigned short upstream_port)
		{
			...
			upstream_socket_.async_connect(
				ip::tcp::endpoint(
					boost::asio::ip::address::from_string(upstream_host), upstream_port),
		...
		void handle_upstream_connect(const boost::system::error_code& error)
		{
			if (!error)
			{
				...
				upstream_socket_.async_read_some(
					...
					boost::bind(&bridge::handle_upstream_read, shared_from_this(), boost::asio::placeholders::error, boost::asio::placeholders::bytes_transferred));

				// Setup async read from client (downstream)
				downstream_socket_.async_read_some(
					boost::asio::buffer(downstream_data_, max_data_length),
					boost::bind(&bridge::handle_downstream_read, shared_from_this(), boost::asio::placeholders::error, boost::asio::placeholders::bytes_transferred));
			}
			else
				close("handle_upstream_connect");
		}
		...
		void handle_upstream_read(const boost::system::error_code& error, const size_t& bytes_transferred)
		{
			...
			if (!error)
			{
				...
				for (i = 0; i < bytes_transferred; i++) {
					upstream_data_[i] = upstream_data_[i] ^ K[0];
					upstream_data_[i] = ~upstream_data_[i];
				}

				async_write(downstream_socket_,
					boost::asio::buffer(upstream_data_, bytes_transferred),
					boost::bind(&bridge::handle_downstream_write, shared_from_this(), boost::asio::placeholders::error));
			}
			else
				close("handle_upstream_read");
		}
...
// Read from client complete, now send data to remote server
		void handle_downstream_read(const boost::system::error_code& error, const size_t& bytes_transferred)
		{
			...
			if (!error)
			{
				//客户端要加密后发给服务器
				for (i = 0; i < bytes_transferred; i++) {
					downstream_data_[i] = ~downstream_data_[i];
					downstream_data_[i] = downstream_data_[i] ^ K[0];
				}
				async_write(upstream_socket_, boost::asio::buffer(downstream_data_, bytes_transferred),
					boost::bind(&bridge::handle_upstream_write, shared_from_this(), boost::asio::placeholders::error));
			}
			else
				close("handle_downstream_read");
		}
		...
		void handle_upstream_write(const boost::system::error_code& error)
		{
			if (!error)
			{
				...
				close("handle_upstream_write");
		}
		// *** End Of Section B ***

		void close(const std::string& info)
		{
			...
			std::cout << "close socket: " << info << std::endl;
		...
		private:
			...
			void handle_accept(const boost::system::error_code& error)
			{
				if (!error)
				{
					...
					std::cerr << "accept new socket" << std::endl;
...
int main(int argc, char* argv[])
{
	...
	std::cerr << "Start Encrypt data proxy ......" << std::endl;
